...
static void mcuzone_parse_voice_cmds(const char* cmd, unsigned int len)
{
    Log::debug("MCUZone: got voice command [%s] of len %d\n", cmd, len);

    if (!strncmp(cmd, "打开LED", len < strlen("打开LED") ? len : strlen("打开LED"))) {
        Log::debug("MCUZone: try to turn on LED\n");
        system("echo none > /sys/class/leds/d5/trigger");
        system("echo 1 > /sys/class/leds/d5/brightness");
    }

    if (!strncmp(cmd, "关闭LED", len < strlen("关闭LED") ? len : strlen("关闭LED"))) {
        Log::debug("MCUZone: try to turn off LED\n");
        system("echo none > /sys/class/leds/d5/trigger");
        system("echo 0 > /sys/class/leds/d5/brightness");
    }
}
...
void MSP_Process(const char* iatResult, void *args)
{
    ...
    mcuzone_parse_voice_cmds(iatResult, strLen);
